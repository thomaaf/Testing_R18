...
sensor_period = 0.01;
% ------------------------------
% HIL param start -> move to separate file later
% ------------------------------

settings.control_system_sample_time = 0.01;
settings.sensor_sampling_time = 0.01;

%CAN IDs

%debug IDs
RESET_INVERTER = hex2dec('353');
DRIVE_ENABLE   = hex2dec('470'); %RTDS finished?

%ECU -> AMK / CM


%%
ECU_SET_POINT_FL_ID = hex2dec('185'); 
ECU_SET_POINT_FR_ID = hex2dec('186'); 
ECU_SET_POINT_RL_ID = hex2dec('189'); 
ECU_SET_POINT_RR_ID = hex2dec('18A'); 

ECU_FZ_DAMPER_EST_ID  = hex2dec('453'); 

listen_ID = [ECU_SET_POINT_FL_ID,ECU_SET_POINT_FR_ID,ECU_SET_POINT_RL_ID,ECU_SET_POINT_RR_ID,800,200,201];

%ADC / Cm / AMK -> ECU


AMK_DATA_ACTUAL_VALUE_1_FL_ID = hex2dec('284');
AMK_DATA_ACTUAL_VALUE_1_FR_ID = hex2dec('285');
AMK_DATA_ACTUAL_VALUE_1_RL_ID = hex2dec('288');
AMK_DATA_ACTUAL_VALUE_1_RR_ID = hex2dec('289');

AMK_DATA_ACTUAL_VALUE_2_FL_ID = hex2dec('286');
AMK_DATA_ACTUAL_VALUE_2_FR_ID = hex2dec('287');
AMK_DATA_ACTUAL_VALUE_2_RL_ID = hex2dec('28a');
AMK_DATA_ACTUAL_VALUE_2_RR_ID = hex2dec('28b');


ACD_DAMPER_FL_ID = hex2dec('400'); 
ACD_DAMPER_FR_ID = hex2dec('401'); 
ACD_DAMPER_RL_ID = hex2dec('402'); 
ACD_DAMPER_RR_ID = hex2dec('403'); 

% ADC_STEERING_ID =                hex2dec('410'); 
% ADC_TORQUE_POSITION_LEFT_ID =    hex2dec('411'); 
% ADC_TORQUE_POSITION_RIGHT_ID =   hex2dec('412'); 
% ADC_KERS_POSITION_ID =           hex2dec('413');
% ADC_BRAKE_POSITION_ID =          hex2dec('414');

ADC_STEERING_ID =                hex2dec('412'); 
ADC_TORQUE_POSITION_LEFT_ID =    hex2dec('411'); 
%ADC_TORQUE_POSITION_RIGHT_ID =   hex2dec('412'); 
ADC_KERS_POSITION_ID =           hex2dec('401');
ADC_BRAKE_POSITION_ID =          hex2dec('414');

BMS_STATE =                     hex2dec('444');
BMS_TS_POWER =                  hex2dec('445');

ECU_STATE =  hex2dec('450');
% emulated INS CAN messages / INS data is normally transmitted on UART
INS_ROLL_ID         =           hex2dec('725'); 
INS_ROLL_RATE_ID    =           hex2dec('711'); 
INS_PITCH_ID        =           hex2dec('712'); 
INS_PITCH_RATE_ID   =           hex2dec('713'); 
INS_YAW_ID          =           hex2dec('714'); 
INS_YAW_RATE_ID     =           hex2dec('720');

INS_V_X_ID           =           hex2dec('723');
INS_V_Y_ID           =           hex2dec('724');
%INS_V_Z_ID           =           hex2dec('777');

INS_A_X_ID           =           hex2dec('721');
INS_A_Y_ID           =           hex2dec('722');
%INS_A_Z_ID           =           hex2dec('777');


% HIL end  start -> move to separate file later
...
settings.use_estimated_Fz = uint32(1); % 1 = yes, 0 = no
...
settings.mu = single(2.5); % 2.5
settings.max_battery_discharge =single(180); % Ampere   |190

i_Diff = 16;   % only GR or include wheel raduis ? ----------------------------- ???
% ------------------------------
% Car Parameters
% ------------------------------
...
car_params.r_eff = single(0.235); %     Continental
...
yawRateControl.Kp_start = single(450); %      450|| 4500 || 1000 skid
yawRateControl.Kp_end = single(450);
yawRateControl.Kp_scaling = single(1); 
  
yawRateControl.Ki_start    = single(2000); % 2000 
...
yawRateControl.Kd_scaling = single(0); %1
...
% ------------------------------
powerLimitControl.positive_power_limit = single(75000); % Positive effect limit
